...
if [ "$color_prompt" = yes ]; then
    PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)──}(%B%F{%(#.red.blue)}%n%(#.💀.@)b0x%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]\n└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
...
alias cat="bat -pp"
alias fm='exploere.exe $1'
alias upg='sudo apt update && sudo apt upgrade && sudo apt dist-upgrade'
alias download='aria2c -s 10 -j 10 -x 16 $1 --file-allocation=none -c'
...
scan() {
    echo -e "[#] Starting scanning on Subnet($1) with Port($2) \n"
    nmap -p $2 --open -oG /tmp/test-$2 $1 -Pn -n 1>/dev/null
    cat /tmp/test-$2 | grep open | cut -d " " -f2 | grep -G "[^Nmap]"
    rm /tmp/test-$2
}
